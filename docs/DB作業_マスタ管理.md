...
| # | ファイル | テーブル | 無いとどうなるか |
|---|---|---|---|
| 1 | `supabase/task_priorities.sql` | `task_priorities` | 優先度タブの保存が失敗。表示は既定値（高/中/低）で正常 |
| 2 | `supabase/task_statuses.sql` | `task_statuses` | 状態タブの保存が失敗。表示は既定値で正常 |
| 3 | `supabase/task_types.sql` | `task_types` | 種別タブの保存が失敗。表示は既定値で正常 |
| 4 | `supabase/task_templates.sql` | `task_templates` | 雛形の**編集**ができない。**生成は既定の雛形で動く** |
| 5 | `supabase/mail_recipients.sql` | `mail_recipients` | 共有先を登録できない。**送信は従来どおり `email_settings` の文字列で動く** |

保存に失敗したときは、画面に原因が出るようにしてあります。

> 優先度マスタのテーブルがまだありません。supabase/task_priorities.sql を Supabase で実行してください。

### ⚠ 上の「保存が失敗」は、タスクの操作とは別のことです

紛らわしいので明記します。**タスク1件の優先度・状態を切り替える操作は、これらのテーブルが無くても動きます。**

| 操作 | 保存先 | 新テーブル |
|---|---|---|
| `/tasks` でタスクの優先度・状態を変える | `tasks` テーブル（**既存**） | **不要** |
| マスタで「高」の呼び名や色を変える | `task_priorities`（**新規**） | **必要** |

`tasks` に保存されるのは `high` / `in_progress` のような**英語のコードだけ**で、
「高」という日本語や色は保存されていません。**コード↔表示の対応表**を
コード内から DB へ移すのが、これらのマスタの役割です。
そのため `tasks` テーブルには一切手を入れておらず、既存データの移行も不要です。

  ...
  try {
    ...
    const knowledgeBlock =
      knowledge.length > 0
        ? "\n\n# 社内資料（参考）\n" +
          ...
          "画面ではあなたの回答の前に、短い一般論と『で、ここからが私の考えなんだけど——』という" +
          "つなぎの一文が表示済みになっている。あなたはその続きとして話す。\n" +
          "- つなぎの続きとして自然につながる書き出しにする（呼びかけ・あいさつ・一般論の繰り返しはしない）。\n" +
          "- 資料に基づく内田さんならではの見方を本題として話す。述べる事実は、必ず上の資料の内容に忠実にする。" +
          "資料に無いことを、資料にあるかのように語らない。資料に無い体験談を作らない。\n" +
        ...
        const g = await ai.models.generateContent({
          ...
          config: {
            systemInstruction:
              ...
              "内田さんの口調のまま1〜3文で簡潔に述べる。『一般的には』のような、一般論だと分かる書き出しにする。" +
              "社内・研修の固有の話には触れない。見出し・前置き・呼びかけは書かず、本文だけを書く。",
          },
        });
    ...
    const stream = new ReadableStream<Uint8Array>({
      async start(controller) {
        // 2段構えの骨組みはサーバが出す（#82）。見出しは会話らしさを壊すため使わず、
        // 「一般論 → つなぎの一文 → 内田さんの考え」を会話の流れでつなぐ（FR-Q3-01）。
        // つなぎ文はコードで挿入するので、モデルの出力形式に依存しない。
        // 一般論が作れなかったときは、つなぎだけが浮くため何も足さず通常の回答に縮退する。
        if (knowledge.length > 0 && generalPart) {
          controller.enqueue(
            encoder.encode(`${generalPart}\n\nで、ここからが私の考えなんだけど——\n\n`),
          );
        }

...
/**
 * 定型タスク — reached from the sidebar. The list sits in the middle of the
 * screen; picking one moves to the generate step.
 *
 * This is deliberately outside the admin guard: templates are defined once by
 * an admin but used by everyone, and a generate button only admins could press
...
export default function TaskTemplatesPage() {
  ...
  // Which step is on screen: the list, or one template's generate form.
  ...
  const childList = (t: TaskTemplate, tone: "light" | "plain") => (
          ...
          {item.taskType && (
            <span
              className={`rounded px-1.5 text-[0.68rem] text-zinc-500 ${
                tone === "light" ? "bg-zinc-100" : "bg-white ring-1 ring-zinc-200"
              }`}
            >
      ...
      <div className="flex flex-col lg:flex-row lg:items-start">
        ...
        <main className="mx-auto w-full max-w-2xl px-4 py-8">
          <div className="mb-6 flex items-center justify-between">
            <h1 className="text-2xl font-bold text-zinc-900">定型タスク</h1>
            <Link
              href="/tasks"
          ...
          {done ? (
            <div className="rounded-2xl bg-white p-6 shadow-sm ring-1 ring-black/5">
              <p className="text-lg font-bold text-[#3B6D11]">
                🌱 「{done.title}」を作成しました
              </p>
            ...
            <div className="rounded-2xl bg-white p-5 shadow-sm ring-1 ring-black/5">
              ...
              {/* What will exist after pressing the button. */}
            ...
            <>
              <p className="mb-4 text-sm text-zinc-600">
                毎回同じ手順で登録している作業の雛形です。選ぶと、
                <b className="text-zinc-800">親タスクと子タスクをまとめて</b>
                登録できます。
              </p>
